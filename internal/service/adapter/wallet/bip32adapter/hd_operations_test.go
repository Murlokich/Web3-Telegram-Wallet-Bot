package bip32adapter //nolint:testpackage // I want to test unexported/private functions
...
func TestGenerateMnemonic(t *testing.T) {
	...
	assert.Len(t, spans, spansGenerateMnemonic)
}
...
func TestGenerateMasterKey(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			mk, err := adapter.deriveMasterKey(tc.mnemonic)
			require.NoError(t, err)
...
func TestGenerateChangeLevelKey(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			changeLevelKey, err := adapter.deriveChangeLevelKey(mk)
			require.NoError(t, err)
...
func TestGetAddress(t *testing.T) { //nolint:tparallel // running in parallel fails to check amount of spans
	t.Parallel()
	tracer, spanRecorder := tracing.NewMockTracer()
	adapter := New(tracer)
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := range tc.addresses {
				computedAddress, err = adapter.GetAddress(ctx, clkBytes, uint32(i))
				require.NoError(t, err)
				assert.Equal(t, tc.addresses[i], computedAddress)
			}
		})
	}
	assert.Len(t, spanRecorder.Ended(), 7)
}
...
func TestGenerateAddress(t *testing.T) {
	...
	ctx := context.Background()
	for range cycles {
		wallet, mnemonic, err := adapter.GenerateHDWallet(ctx, 0)
		require.NoError(t, err)
		words := strings.Split(mnemonic, " ")
		assert.Len(t, words, wordSetLength)
		assert.Len(t, wallet.MasterKey, privateKeyBytesLength)
		assert.Len(t, wallet.AddressManagementData.ChangeLevelKey, privateKeyBytesLength)
	}
	assert.Len(t, spanRecorder.Ended(), 2*cycles)
}
