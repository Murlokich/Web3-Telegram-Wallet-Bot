...
func AddNewAddress(ctx context.Context, conn *pgx.Conn, userID int64) (*encryption.EncryptedEntry, uint32, error) {
	query := `
	...
	if err := conn.QueryRow(ctx, query, userID).Scan(&entry.Ciphertext, &entry.Nonce, &lastAddressIndex); err != nil {
